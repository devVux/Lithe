...
struct RenderKey {
	...
	RenderKey(MaterialID materialID, TextureID textureID, MeshID meshID) {
		key = (uint64_t(materialID) << (TEXTURE_BITS + MESH_BITS)) |
			(uint64_t(textureID) << MESH_BITS) |
			uint64_t(meshID);
	}
...
class RenderCache {
	...
	public:
		...
		[[nodiscard]] bool hasMesh(MeshID id) const noexcept {
			return mMeshAllocations.find(id) != mMeshAllocations.end();
		}
		...
		[[nodiscard]] bool addKey(const RenderKey& key) noexcept {
			auto it = std::upper_bound(mKeys.begin(), mKeys.end(), key, 
				[](const RenderKey& a, const RenderKey& b) { return a < b; }
			);
