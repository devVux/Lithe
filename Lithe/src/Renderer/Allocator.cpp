...
Allocator::Allocator(Allocator&& other) noexcept
	: mAllocator(std::exchange(other.mAllocator, VK_NULL_HANDLE)), mBuffers(std::move(other.mBuffers)) { }
...
Image Allocator::createImage(uint32_t w, uint32_t h, LtFormat format, VkBufferUsageFlags usage) noexcept {
	...
	return mImages.emplace_back(Image {.handle = image, .allocation = imageAllocation});
}
...
Image Allocator::createDepthImage(uint32_t width, uint32_t height, uint32_t depth) noexcept {
	...
	return mImages.emplace_back(Image {.handle = depthImage, .allocation = depthAllocation});
}
