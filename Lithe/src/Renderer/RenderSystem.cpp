...
bool RenderSystem::init(EventDispatcher& dispatcher, ISurface& surface, std::set<Extension> extensions) noexcept {
		...
		result =
			Validation::createInstance(extensions, layers, hasValidationLayer)
				...
				.and_then([&](InitContext ctx) -> std::expected<InitContext, E> {
					...
					auto [descriptor, layout, pipe] =
						Pipeline::createPipeline(mDevice, ctx.extent, ctx.surfaceFormat.format);
					...
					mDescriptorSetLayouts = RAIIed<std::vector<VkDescriptorSetLayout>>(
						descriptor,
						[device =
							 static_cast<VkDevice>(mDevice)](std::vector<VkDescriptorSetLayout> setLayouts) noexcept {
					...
					mPipeline = RAIIed<VkPipeline>(
						*pipe, [device = static_cast<VkDevice>(mDevice)](VkPipeline pipeline) noexcept {
							if (pipeline)
				...
				.and_then([&](InitContext ctx) -> std::expected<InitContext, E> {
					...
					// TODO: improve cmd buffers count
					allocInfo.commandBufferCount = static_cast<uint32_t>(mCommandBuffers.size()) * 2;

					std::vector<VkCommandBuffer> flat(nFramesInFlight * 2);

					if (vkAllocateCommandBuffers(mDevice, &allocInfo, flat.data()) != VK_SUCCESS)
						return std::unexpected {E::Unknown};

					for (uint32_t i = 0; i < nFramesInFlight; ++i) {
						mCommandBuffers[i].scene   = flat[i * 2 + 0];
						mCommandBuffers[i].overlay = flat[i * 2 + 1];
					}
	...
	swapchainExtent.width  = result->extent.width;
	swapchainExtent.height = result->extent.height;

	debugLayer.init(
		dispatcher, mInstance, mPhysicalDevice, mDevice, mGraphicsQueue, mIndices.graphicsFamily.value(),
		(LtFormat) result->surfaceFormat.format, 2, mSwapchainImages.size(),
		{swapchainExtent.width, swapchainExtent.height}
	);

	mAllocator = Allocator(mPhysicalDevice, mDevice, mInstance);

	mVertexBuffer = mAllocator.createBuffer(
		1_MB, VK_BUFFER_USAGE_VERTEX_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT, Mapped::Yes
	);

	mIndexBuffer =
		mAllocator.createBuffer(1_MB, VK_BUFFER_USAGE_INDEX_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT, Mapped::Yes);

	mMaterialBuffer = mAllocator.createBuffer(
		10 * sizeof(Render::MaterialData), VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT,
		Mapped::Yes
	);

	mStorageBuffer = mAllocator.createBuffer(
		100 * sizeof(Render::InstanceData), VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT,
		Mapped::Yes
	);

	mUniformBuffers.resize(nFramesInFlight);
	for (size_t i = 0; i < nFramesInFlight; i++)
		mUniformBuffers[i] = mAllocator.createBuffer(
			sizeof(Render::CameraUBO), VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT,
			Mapped::Yes
		);
	...
	for (int i = 0; i < mSwapchainImages.size(); i++) {
		...
		VkSemaphore sceneRendererdSemaphore;
		vkCreateSemaphore(mDevice, &semInfo, NULL, &sceneRendererdSemaphore);
		mSceneRenderedSemaphore.push_back(
			RAIIed<VkSemaphore>(
				sceneRendererdSemaphore, [device = static_cast<VkDevice>(mDevice)](auto semaphore) noexcept {
					if (semaphore)
						vkDestroySemaphore(device, semaphore, nullptr);
				}
			)
	...
	std::vector<VkDescriptorPoolSize> poolSizes {
		// set 0
		{.type = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, .descriptorCount = static_cast<uint32_t>(nFramesInFlight)},
	...
	for (auto i = 0; i < nFramesInFlight; i++) {
		...
		mDepthImageViews[i] =
			RAIIed<VkImageView>(depthImageView, [device = static_cast<VkDevice>(mDevice)](auto view) noexcept {
				if (view)
					vkDestroyImageView(device, view, nullptr);
			});
	}
...
bool RenderSystem::uploadStaticData(StaticRenderPacket& statics, IResourceCache& cache) noexcept {
	...
	for (const auto& [id, material] : cache.materials()) {
		...
		materials.emplace_back(
			Render::MaterialData {
				.color = material.color, .albedoTextureIndex = static_cast<uint32_t>(material.albedoTextureID)
			}
		);
	...
	vmaCreateBuffer(
		tempAllocator, &bufferInfo, &allocInfo, &stagingBuffer.handle, &stagingBuffer.allocation, &allocationInfo
	);
	...
	for (const auto& [id, texture] : cache.textures()) {
		...
		auto image = mAllocator.createImage(
			texture.width, texture.height, LT_FORMAT_R8G8B8A8_SRGB,
		...
		vkCmdPipelineBarrier(
			transferCmd, VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, 0, 0, nullptr, 0, nullptr,
			1, &barrierToDst
		);
		...
		vkCmdCopyBufferToImage(
			transferCmd, stagingBuffer.handle, image.handle, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &region
		);
		...
		vkCmdPipelineBarrier(
			transferCmd, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT, 0, 0, nullptr, 0,
			nullptr, 1, &barrierToShader
		);
	...
	for (const auto& entity : statics.instances) {
		...
		instances.emplace_back(
			Render::InstanceData {
				.model = entity.model, .materialIndex = entity.materialID, .textureIndex = entity.textureID
			}
		);
...
void RenderSystem::render(DynamicRenderPacket& dynamics, IResourceCache& cache) noexcept {
	debug.reset();
	debug.delta = clock.timeSinceLastUpdate();

	auto cmd = mCommandBuffers.at(currentFrame).scene;
	...
	vkCmdPipelineBarrier(
		cmd, VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT, VK_PIPELINE_STAGE_EARLY_FRAGMENT_TESTS_BIT, 0, 0, nullptr, 0, nullptr,
		1, &depthBarrier
	);
	...
	vkCmdBindDescriptorSets(
		cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, mPipelineLayout, 0, mDynamicDescriptorSets[currentFrame].size(),
		mDynamicDescriptorSets[currentFrame].data(), 0, nullptr
	);

	vkCmdBindDescriptorSets(
		cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, mPipelineLayout, 1,
		mPersistentDescriptorSets.size() - 1, // except set 0
		mPersistentDescriptorSets.data() + 1, 0, nullptr
	);
	...
	while (firstIstance < renderCache.keys().size()) {
		...
		vkCmdDrawIndexed(
			cmd, meshAlloc.indexCount, instanceCount, meshAlloc.firstIndex, meshAlloc.vertexOffset, firstIstance
		);

		debug.drawCalls		+= 1;
		debug.instanceCount += static_cast<uint32_t>(instanceCount);

		if (instanceCount > 1)
			debug.mergedBatches += 1; // count this batch as merged

		debug.triangleCount += meshAlloc.indexCount / 3 * instanceCount;
		debug.vertexCount	+= meshAlloc.vertexCount * instanceCount;

		debug.materialDrawCalls[materialID] += 1;
		debug.meshDrawCalls[meshID]			+= 1;
	...
	submitInfo.pSignalSemaphores	  = &static_cast<const VkSemaphore&>(mSceneRenderedSemaphore[currentFrame]);
...
void RenderSystem::renderOverlay() noexcept {
	auto cmd = mCommandBuffers.at(currentFrame).overlay;
	...
	vkCmdPipelineBarrier(
		cmd, VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT, VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT, 0, 0,
	...
	VkPipelineStageFlags waitStages[] = {VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT};
	submitInfo.waitSemaphoreCount	  = 1;
	submitInfo.pWaitSemaphores		  = &static_cast<const VkSemaphore&>(mSceneRenderedSemaphore[currentFrame]);
	submitInfo.pWaitDstStageMask	  = waitStages;
	submitInfo.commandBufferCount	  = 1;
	submitInfo.pCommandBuffers		  = &cmd;
	submitInfo.signalSemaphoreCount	  = 1;
